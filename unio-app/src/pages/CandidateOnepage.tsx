...
<body>
  ...
  const UNLOCK_ORDER = ['prescreening','entrevistas','evaluaciones','prueba_conocimiento','estudios','finalistas'] as const;
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          {stage !== 'scoring' && stage !== 'prescreening' && stage !== 'finalistas' && stage !== 'estudios' && (
            <Button
              ...
              >
                <CheckCircle2 size={16} />
                {stage === 'prescreening' ? 'Pasar a Entrevista'
    ...
    <div style={{ paddingTop: '20px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        >
          ...
          {/* Standard rows */}
          {[
            { label: 'Mín. 6 meses de experiencia certificada en servicio al cliente (con NIT y certificable es suficiente).', fails: false },
            { label: 'Edad máx. 33–34 años.', fails: prescreeningRechazado },
            { label: 'Experiencia en restaurante de cadena (deseable; no obligatorio si hay actitud y certificación).', fails: false },
            { label: 'Disponibilidad completa para turnos rotativos (lunes a domingo).', fails: false },
          ].map(({ label, fails }, i) => (
            <div
              key={i}
              style={{
          ...
          <div
            style={{
              ...
              gridTemplateColumns: '1fr 120px',
              alignItems: 'center',
              background: (!isPendingEvaluaciones && antecedentesAltoRiesgo) ? '#fff5f5' : undefined,
            }}
          >
            <div style={{ padding: '14px 24px', fontFamily: 'var(--font-display)', fontWeight: 500, fontSize: '13px', lineHeight: '21px', color: isPendingEvaluaciones ? '#9ca3af' : antecedentesAltoRiesgo ? '#b91c1c' : '#363539' }}>
