...
function getContexto(cargo: string): { priorities: string[]; autonomy: string } {
  ...
  if (c.includes('odonto')) return {
    priorities: ['Atención pediátrica preventiva con excelencia clínica', 'Educación en salud oral a familias de la red Amore Group Talent', 'Reducir incidencia de caries en grupo 0-6 años en 20%'],
    autonomy: 'Media',
  };
  if (c.includes('bdm') || c.includes('business') || c.includes('comercial')) return {
    priorities: ['Captar 12+ empresas afiliadas nuevas en el semestre', 'Crecer cartera de cuentas activas en 30%', 'Posicionar servicios Amore Group Talent en sector empresarial del Valle'],
    autonomy: 'Alta',
  };
  return {
    priorities: ['Cumplir los indicadores del área asignada', 'Fortalecer la calidad del servicio al afiliado', 'Apoyar la estrategia de bienestar de Amore Group Talent'],
    autonomy: 'Media',
  };
}
      ...
      <div style={{ minHeight: '100vh', background: 'var(--color-surface-subtle)', fontFamily: 'var(--font-display)' }}>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <img src={assetUrl('/logo-amore-group.png')} alt="Amore Group Talent" style={{ maxHeight: '100px', maxWidth: '300px', width: 'auto', height: 'auto', objectFit: 'contain' }} />
