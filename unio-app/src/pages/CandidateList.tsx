...
const STAGE_CHIPS: Record<string, ChipDef[]> = {
  scoring: SCORING_CHIPS, prescreening: PRESCREENING_CHIPS, evaluaciones: SCORING_CHIPS, prueba_conocimiento: SCORING_CHIPS,
...
const SCORING_STAGES = new Set(['scoring', 'prescreening', 'evaluaciones', 'prueba_conocimiento']);
...
export default function CandidateList() {
  ...
  const STAGE_ORDER = ['scoring', 'prescreening', 'entrevistas', 'evaluaciones', 'prueba_conocimiento', 'estudios', 'finalistas'] as const;
  ...
  const currentStage = (() => {
    ...
    return stage;
  })() as 'scoring' | 'prescreening' | 'entrevistas' | 'evaluaciones' | 'prueba_conocimiento' | 'estudios' | 'finalistas';
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "transparent" }}>
      ...
      <WizardBar visible={selected.size > 0}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          {currentStage !== 'scoring' && currentStage !== 'prescreening' && currentStage !== 'finalistas' && currentStage !== 'estudios' && currentStage !== 'entrevistas' && (
            ...
            >
              <CheckCircle2 size={18} />
              {currentStage === 'prescreening' ? 'Pasar a Entrevista'
      ...
      <WhatsAppAgendarEntrevistaModal
        ...
        onConfirmSend={(cands) => {
          ...
          if (nextStage) setProgressStage(nextStage as Parameters<typeof setProgressStage>[0]);
          setSelected(new Set());
          setToastMessage(`${ids.length} candidato${ids.length !== 1 ? 's' : ''} agendado${ids.length !== 1 ? 's' : ''} para entrevista`);
          setToastVisible(true);
        }}
      />
