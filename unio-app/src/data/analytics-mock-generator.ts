...
export function generateAnalyticsData(
  ...
  const funnelStages: FunnelStage[] = [
    ...
    { label: 'ENTREVISTA',       value: entrevistas, pct: Math.round(entrevistas / aplicados * 100),             convLabel: `${Math.round(entrevistas/prescreen*100)}% conversión`, dropoff: null },
  ...
  const dropoffTable: DropoffRow[] = [
    { reason: 'Sin experiencia certificada en servicio al cliente', count: c1, pct: `${p1}%` },
    { reason: 'Sin disponibilidad para turnos rotativos',           count: c2, pct: `${p2}%` },
  ...
  const timePhases: TimePhase[] = [
    ...
    { label: 'Entrevista',         days: phaseDays[2], bottleneck: bottleneckIdx === 2 },
  ...
  const hrResponse: HrResponseRow[] = [
    ...
    { from: 'Pre-screening → Entrevista',     days: hrDays[1], warning: hrDays[1] === hrMax },
    { from: 'Entrevista → Val. Documental',  days: hrDays[2], warning: hrDays[2] === hrMax },
    { from: 'Val. Documental → Shortlist',   days: hrDays[3], warning: hrDays[3] === hrMax },
  ];

  // ── Aging crítico
  const VACANTES_AGING = [
    { vacante: 'Mesero / Polifuncional – Medellín', area: 'Operaciones' },
    { vacante: 'Auxiliar de Cocina – Bogotá',       area: 'Cocina' },
    { vacante: 'Cajero / Domiciliario',             area: 'Operaciones' },
    { vacante: 'Bartender / Barra',                 area: 'Barra' },
    { vacante: 'Jefe de Cocina – Turno Noche',      area: 'Cocina' },
    { vacante: 'Host / Hostess',                    area: 'Servicio' },
  ];
  ...
  // ── Detail Table
  const VACANTE_POOL = [
    { vacante: 'Mesero / Polifuncional – Medellín',   area: 'Operaciones',  tipo: 'Operativa' },
    { vacante: 'Auxiliar de Cocina – Bogotá',          area: 'Cocina',       tipo: 'Operativa' },
    { vacante: 'Cajero / Domiciliario',                area: 'Operaciones',  tipo: 'Operativa' },
    { vacante: 'Bartender / Barra',                    area: 'Barra',        tipo: 'Operativa' },
    { vacante: 'Jefe de Cocina – Turno Noche',         area: 'Cocina',       tipo: 'Operativa' },
    { vacante: 'Host / Hostess',                       area: 'Servicio',     tipo: 'Operativa' },
    { vacante: 'Supervisor de Turno',                  area: 'Operaciones',  tipo: 'Administrativa' },
    { vacante: 'Coord. de Calidad e Higiene',          area: 'HSEQ',         tipo: 'Administrativa' },
    { vacante: 'Despachador de Pedidos',               area: 'Operaciones',  tipo: 'Operativa' },
    { vacante: 'Coord. Apertura Nuevos Puntos',        area: 'Expansión',    tipo: 'Estratégica' },
  ];
